...
export function checkCommandPermissions(
  ...
  for (const cmd of commandsToValidate) {
    // PERMANENT SAFETY BLOCK: Prevent recursive removal
    if (
      cmd.match(/\brm\s+-[^\s]*r[^\s]*f\b/) ||
      cmd.match(/\brm\s+-[^\s]*f[^\s]*r\b/) ||
      cmd.match(/\brm\s+.*-r.*\b/)
    ) {
      return {
        allAllowed: false,
        disallowedCommands: [cmd],
        blockReason: `CRITICAL SAFETY VIOLATION: Recursive removal (rm -r/-rf) is permanently blocked by core protocol.`,
        isHardDenial: true,
      };
    }

    // PERMANENT SAFETY BLOCK: Prevent .git deletion
    if (cmd.match(/\brm\b.*\.git\b/) || cmd.match(/\brmdir\b.*\.git\b/)) {
      return {
        allAllowed: false,
        disallowedCommands: [cmd],
        blockReason: `CRITICAL SAFETY VIOLATION: Deletion of .git directories is permanently blocked by core protocol. These are sacred Heartwood infrastructure.`,
        isHardDenial: true,
      };
    }
