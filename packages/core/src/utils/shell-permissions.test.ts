...
describe('isCommandAllowed', () => {
  ...
  it('should block grep, rg, and ripgrep with a hard denial', () => {
    const grepResult = checkCommandPermissions(
      'grep "pattern" file.txt',
      config,
    );
    expect(grepResult.allAllowed).toBe(false);
    expect(grepResult.isHardDenial).toBe(true);
    expect(grepResult.blockReason).toContain('CRITICAL SAFETY VIOLATION');

    const rgResult = checkCommandPermissions('rg "pattern"', config);
    expect(rgResult.allAllowed).toBe(false);
    expect(rgResult.isHardDenial).toBe(true);

    const ripgrepResult = checkCommandPermissions('ripgrep "pattern"', config);
    expect(ripgrepResult.allAllowed).toBe(false);
    expect(ripgrepResult.isHardDenial).toBe(true);
  });
...
EOF`,
    ...
    expect(result.allowed).toBe(false);
    expect(result.reason).toBe(
      `CRITICAL SAFETY VIOLATION: Recursive removal (rm -r/-rf) is permanently blocked by core protocol.`,
    );
  });
