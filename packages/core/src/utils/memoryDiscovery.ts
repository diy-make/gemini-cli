...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    // Every technical instruction must be explicitly authorized via the Memory Framework.
...
export async function getGlobalMemoryPaths(): Promise<string[]> {
  // Legislative standard: Return empty set for global .gemini discovery.
  // We prioritize the specialized Memory Framework Books over generic global files.
  return [];
}
